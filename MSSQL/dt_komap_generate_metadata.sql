...
insert into ACT_RESEARCH_V41 (C_HLEVEL, C_FULLNAME, C_NAME, C_SYNONYM_CD, C_VISUALATTRIBUTES, C_TOTALNUM, C_BASECODE, C_METADATAXML, C_FACTTABLECOLUMN, C_TABLENAME, C_COLUMNNAME, C_COLUMNDATATYPE, C_OPERATOR, C_DIMCODE, C_COMMENT, C_TOOLTIP, M_APPLIED_PATH, UPDATE_DATE, DOWNLOAD_DATE, IMPORT_DATE, SOURCESYSTEM_CD, VALUETYPE_CD, M_EXCLUSION_CD, C_PATH, C_SYMBOL)
	select 4 C_HLEVEL, 
		...
		'@' M_APPLIED_PATH, 
		cast(GetDate() as date) UPDATE_DATE,
		cast(GetDate() as date) DOWNLOAD_DATE,
...
insert into ACT_RESEARCH_V41 (C_HLEVEL, C_FULLNAME, C_NAME, C_SYNONYM_CD, C_VISUALATTRIBUTES, C_TOTALNUM, C_BASECODE, C_METADATAXML, C_FACTTABLECOLUMN, C_TABLENAME, C_COLUMNNAME, C_COLUMNDATATYPE, C_OPERATOR, C_DIMCODE, C_COMMENT, C_TOOLTIP, M_APPLIED_PATH, UPDATE_DATE, DOWNLOAD_DATE, IMPORT_DATE, SOURCESYSTEM_CD, VALUETYPE_CD, M_EXCLUSION_CD, C_PATH, C_SYMBOL)
	select C_HLEVEL-1 C_HLEVEL, 
		...
		(case when right(rtrim(C_NAME),1)=')' then left(C_NAME,len(rtrim(C_NAME))-1)+', ' else rtrim(C_NAME)+' (' end) + 'Probabilistic, Not Validated)' C_NAME,
		C_SYNONYM_CD, 
		C_VISUALATTRIBUTES,
		C_TOTALNUM, 
	...
	where c_fullname like '\ACT\Research\Phenotyping\PheCode\Case\_%'
		and (c_basecode is null or c_basecode not like 'ICD%'); -- Do not include the ICD mappings
...
update ACT_RESEARCH_V41
	...
	where C_FULLNAME like '\ACT\Research\Phenotyping\PheCodeDTNotValidated\%'
		and C_FULLNAME not in (
			select left(C_FULLNAME,len(C_FULLNAME)-charindex('\',reverse(C_FULLNAME),2)+1) PARENT_FOLDER
			from ACT_RESEARCH_V41
			where C_FULLNAME like '\ACT\Research\Phenotyping\PheCodeDTNotValidated\%'		
		);
...
insert into ACT_RESEARCH_V41 (C_HLEVEL, C_FULLNAME, C_NAME, C_SYNONYM_CD, C_VISUALATTRIBUTES, C_TOTALNUM, C_BASECODE, C_METADATAXML, C_FACTTABLECOLUMN, C_TABLENAME, C_COLUMNNAME, C_COLUMNDATATYPE, C_OPERATOR, C_DIMCODE, C_COMMENT, C_TOOLTIP, M_APPLIED_PATH, UPDATE_DATE, DOWNLOAD_DATE, IMPORT_DATE, SOURCESYSTEM_CD, VALUETYPE_CD, M_EXCLUSION_CD, C_PATH, C_SYMBOL)
	select 5 C_HLEVEL,
		...
		(case when right(rtrim(C_NAME),1)=')' then left(C_NAME,len(rtrim(C_NAME))-1)+', ' else rtrim(C_NAME)+' (' end) + 'Probabilistic, PPV=TBD, Recall=TBD)' C_NAME,
		C_SYNONYM_CD, 
